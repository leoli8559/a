...
/*-------- Service class 0x00: administrative services -------------*/
    ...
    #define HSE_SRV_ID_BOOT_DATA_IMAGE_SIGN     ((hseSrvId_t)(HSE_SRV_VER_0 | 0x00000008UL))  /**< @brief Boot Data image sign (e.g. for HSE-H/M, IVT/DCD/ST/LPDDR4(S32Z/E devices)/AppBSB image; for HSE-B, IVT/AppBSB image). Data structure used: #hseBootDataImageSignSrv_t. */
    #define HSE_SRV_ID_BOOT_DATA_IMAGE_VERIFY   ((hseSrvId_t)(HSE_SRV_VER_0 | 0x00000009UL))  /**< @brief Boot Data images verify (e.g. for HSE-H/M, IVT/DCD/ST/LPDDR4(S32Z/E devices)/AppBSB image; for HSE-B, IVT/AppBSB image). Data structure used: #hseBootDataImageVerifySrv_t. */
    ...
    #ifdef HSE_SPT_MONOTONIC_COUNTERS
    #define HSE_SRV_ID_PUBLISH_LOAD_CNT_TBL     ((hseSrvId_t)(HSE_SRV_VER_0 | 0x00000013UL))  /**< @brief Request to publish/load the NVM container for the Monotonic Counter table (only for HSE-H). Data structure used: #hsePublishLoadCntTblSrv_t. */
    #endif /* HSE_SRV_ID_PUBLISH_LOAD_CNT_TBL*/

    #ifdef HSE_SPT_OTFAD
    #define HSE_SRV_ID_INSTALL_OTFAD_CTX        ((hseSrvId_t)(HSE_SRV_VER_0 | 0x00000014UL))  /**< @brief Install an On-The-Fly AES Decryption (OTFAD) context (only for HSE-H). Data structure used: #hseInstallOtfadContextSrv_t. */
    #define HSE_SRV_ID_ACTIVATE_OTFAD_CTX       ((hseSrvId_t)(HSE_SRV_VER_0 | 0x00000015UL))  /**< @brief Activate on-demand OTFAD context (only for HSE-H). Data structure used: #hseActivateOtfadContextSrv_t. */
    #define HSE_SRV_ID_GET_OTFAD_CTX            ((hseSrvId_t)(HSE_SRV_VER_0 | 0x00000016UL))  /**< @brief Get OTFAD context information (only for HSE-H). Data structure used: #hseGetOtfadContextSrv_t. */
    #endif /* HSE_SPT_OTFAD */

    #define HSE_SRV_ID_PREPARE_FOR_STANDBY      ((hseSrvId_t)(HSE_SRV_VER_0 | 0x00A50017UL))  /**< @brief Prepare HSE before system goes to Stand-By mode (only for HSE-H/HSE-M). Data structure used: #hsePrepareForStandBySrv_t. */
    ...
    #define HSE_SRV_ID_ERASE_HSE_NVM_DATA       ((hseSrvId_t)(HSE_SRV_VER_0 | 0x00000050UL))  /**< @brief Erase HSE Data Flash (only for HSE-B). This service is only allowed in CUST_DEL LC. Data structure used: #hseEraseNvmDataSrv_t. */
...
#ifdef HSE_SPT_MONOTONIC_COUNTERS
    #define HSE_SRV_ID_CONFIG_COUNTER           ((hseSrvId_t)(HSE_SRV_VER_0 | 0x00000052UL))  /**< @brief Configure the secure counter (only for HSE-B). This service requires SuperUser rights. Data structure used: #hseConfigSecCounterSrv_t. */
    ...
    #define HSE_SRV_ID_FW_INTEGRITY_CHECK       ((hseSrvId_t)(HSE_SRV_VER_0 | 0x00000054UL))  /**< @brief Application request to check HSE flash memory integrity (only for HSE-B). No data structure used. */
...
typedef struct
{
    ...
    union
    {
        ...
        #ifdef HSE_SPT_BOOTDATASIGN
            hseBootDataImageSignSrv_t       bootDataImageSignReq;   /**< @brief Request to generate the Signature  for Boot Data images (e.g. for HSE-H/M, IVT/DCD/ST/LPDDR4(ZSE devices)/AppBSB image; for HSE-M/B, IVT/XRDC/AppBSB image) */
            hseBootDataImageVerifySrv_t     bootDataImageSigVerifyReq;/**< @brief Request to verify the Signature for Boot Data images (e.g. for HSE-H/M, IVT/DCD/ST/LPDDR4(ZSE devices)/AppBSB image; for HSE-M/B, IVT/XRDC/AppBSB image) */
        #endif
        #ifdef HSE_SPT_STREAM_CTX_IMPORT_EXPORT
